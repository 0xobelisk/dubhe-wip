  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (errorData.errors?.[0]?.message?.includes('Syntax Error')) {
        throw new Error(`GraphQL syntax error: ${errorData.errors[0].message}`);
      }

      if (errorData.errors?.length > 0) {
        throw new Error(errorData.errors[0].message || 'Unknown GraphQL error');
      }

      throw new Error(`HTTP error: ${JSON.stringify(errorData)} (status: ${response.status})`);
    }
