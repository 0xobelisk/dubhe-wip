module dubhe::dubhe_dex_functions {
    ...
    use dubhe::dubhe_pools::Pools;
    use dubhe::dubhe_config;
    use dubhe::dubhe_asset_metadata::Asset_metadata;
    use dubhe::dubhe;
    ...
    public(package) fun get_pool(dapp_hub: &DappHub, asset_a: address, asset_b: address): Pools {
        let (asset_0, asset_1) = sort_assets(asset_a, asset_b);
        dubhe_pools::ensure_has(dapp_hub, asset_0, asset_1);
        dubhe_pools::get_struct(dapp_hub, asset_0, asset_1)
    }

    public(package) fun get_reserves(dapp_hub: &DappHub, asset_a: address, asset_b: address): (u256, u256) {
        let (asset_0, asset_1) = sort_assets(asset_a, asset_b);
        dubhe_pools::ensure_has(dapp_hub, asset_0, asset_1);
        let pool = dubhe_pools::get_struct(dapp_hub, asset_0, asset_1);
    ...
    public(package) fun pool_asset_symbol(asset1_metadata: Asset_metadata, asset2_metadata: Asset_metadata): vector<u8> {
    ...
    public(package) fun update(pool: &mut Pools, balance0: u256, balance1: u256) {
    ...
    public(package) fun mint_fee(dapp_hub: &mut DappHub, pool: &mut Pools, reserve0: u256, reserve1: u256): bool {
        ...
        if (fee_on) {
            if (k_last != 0) {
                    ...
                    if (root_k > root_k_last) {
                        let total_supply = dubhe_assets_system::supply_of(dapp_hub, pool.lp_asset());
                        let numerator = total_supply * (root_k - root_k_last);
                        let denominator = root_k * 5 + root_k_last;
                        let liquidity = numerator / denominator;
                        if (liquidity > 0) dubhe_assets_functions::do_mint(dapp_hub, pool.lp_asset(), fee_to, liquidity);
                    }
            }
        } else if (k_last != 0) {
    ...
    public(package) fun mint(dapp_hub: &mut DappHub, asset_a: address, asset_b: address, to: address, ctx: &TxContext): u256 {
        ...
        let balance0 = dubhe_assets_system::balance_of(dapp_hub, asset_0, pool.pool_address());
        let balance1 = dubhe_assets_system::balance_of(dapp_hub, asset_1, pool.pool_address());
        ...
        let liquidity = if (total_supply == 0) {
            ...
            dubhe_assets_functions::do_mint(dapp_hub, pool.lp_asset(), @0x0, MINIMUM_LIQUIDITY); 
        ...
        dubhe_assets_functions::do_mint(dapp_hub, pool.lp_asset(), to, liquidity);
        ...
        dubhe_pools::set_struct(dapp_hub, asset_0, asset_1, pool);
        liquidity
    }
    ...
    public(package) fun burn(dapp_hub: &mut DappHub, asset_a: address, asset_b: address, to: address, ctx: &TxContext) : (u256, u256) {
        ...
        let balance0 = dubhe_assets_system::balance_of(dapp_hub, asset_0, pool.pool_address());
        let balance1 = dubhe_assets_system::balance_of(dapp_hub, asset_1, pool.pool_address());
        let liquidity = dubhe_assets_system::balance_of(dapp_hub, pool.lp_asset(), pool.pool_address());
        ...
        dubhe_assets_functions::do_burn(dapp_hub, pool.lp_asset(), pool.pool_address(), liquidity);
        dubhe_assets_functions::do_transfer(dapp_hub, asset_0, pool.pool_address(), to, amount0);
        dubhe_assets_functions::do_transfer(dapp_hub, asset_1, pool.pool_address(), to, amount1);
    ...
    public(package) fun swap(
        ...
        if (amount0_out > 0) dubhe_assets_functions::do_transfer(dapp_hub, asset_0, pool.pool_address(), to, amount0_out); // optimistically transfer tokens
        if (amount1_out > 0) dubhe_assets_functions::do_transfer(dapp_hub, asset_1, pool.pool_address(), to, amount1_out); // optimistically transfer tokens
        let balance0 = dubhe_assets_system::balance_of(dapp_hub, asset_0, pool.pool_address());
        let balance1 = dubhe_assets_system::balance_of(dapp_hub, asset_1, pool.pool_address());
