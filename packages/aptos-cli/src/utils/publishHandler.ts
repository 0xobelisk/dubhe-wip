...
export async function publishHandler(
  ...
  try {
    ...
    console.log(chalk.green(`Publish Transaction Digest: ${response.hash}`));
  } catch (error: any) {
  ...
  try {
    ...
    console.error(
      chalk.red(`Failed to execute deployHook, please republish or manually call deploy_hook::run`)
    );
    console.error(error.message);
    process.exit(1);
  }
