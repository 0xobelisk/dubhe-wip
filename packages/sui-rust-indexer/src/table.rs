...
impl TableMetadata {
    ...
    pub fn generate_create_table_sql(&self) -> String {
        ...
        for field in &self.fields {
                fields.push(format!("{} {}", field.field_name, self.get_sql_type(&field.field_type)));
        }
        ...
        if self.fields.iter().any(|field| field.is_key) {
            let key_names: Vec<String> = self.fields.iter()
            ...
            let value_names: Vec<String> = self.fields.iter()
                .filter(|field| !field.is_key)
                .map(|field| field.field_name.clone())
                .collect();
    ...
    pub fn parse_table_field(&self, field_name: &Vec<u8>, field_type: &Vec<u8>, field_value: &[u8]) -> Value {
    ...
    pub fn parse_table_keys(&self, keys: Vec<Vec<u8>>) -> Value {
        ...
        self.fields.iter().for_each(|field| {
            if field.is_key {
                let field_json = self.parse_table_field(&field.field_name.as_bytes().to_vec(), &field.field_type.as_bytes().to_vec(), &keys[key_index]);
    ...
    pub fn parse_table_values(&self, values: Vec<Vec<u8>>) -> Value {
        ...
        self.fields.iter().for_each(|field| {
            if !field.is_key {
                let field_json = self.parse_table_field(&field.field_name.as_bytes().to_vec(), &field.field_type.as_bytes().to_vec(), &values[value_index]);
                result[field.field_name.clone()] = field_json;
                value_index += 1;
            }
        });
...
mod tests {
    ...
    fn test_generate_create_table_sql() {
        ...
        let (package_id, tables) = TableMetadata::from_json(test_json).unwrap();
            ...
            let table = &tables[0];
            assert_eq!(
                table.generate_create_table_sql(), 
                "CREATE TABLE IF NOT EXISTS store_counter0 (entity_id TEXT, created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (entity_id))"
            );
            assert_eq!(
                table.generate_insert_table_fields_sql(), 
                vec![
            ...
            assert_eq!(
                table.generate_create_table_sql(), 
                "CREATE TABLE IF NOT EXISTS store_counter1 (entity_id TEXT, value BIGINT, created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (entity_id))"
            );
            assert_eq!(
                table.generate_insert_table_fields_sql(), 
                vec![
            ...
            assert_eq!(
                table.generate_create_table_sql(), 
                "CREATE TABLE IF NOT EXISTS store_counter2 (value BIGINT, created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (value))"
            );
            assert_eq!(
                table.generate_insert_table_fields_sql(), 
                vec![
