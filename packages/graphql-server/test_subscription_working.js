...
async function testRealSubscription() {
  ...
  try {
    // 验证 createClient 是否正确导入
    if (typeof createClient === 'undefined') {
      throw new Error('createClient 导入失败');
    }
    
    console.log('✅ createClient 导入成功');
    ...
    const dispose1 = client.subscribe(
      ...
      },
      {
        ...
        error: (error) => {
          ...
          console.error('   可能原因:');
          console.error('   1. GraphQL 服务器未启动');
          console.error('   2. subscription 功能未启用');
          console.error('   3. schema 中没有对应的订阅');
        },
        complete: () => {
          console.log('✅ 订阅完成');
        },
    ...
    setTimeout(() => {
      ...
      console.log('   1. GraphQL 服务器是否正常运行在 http://localhost:4000');
      console.log('   2. 运行 python test_notify.py 来发送测试通知');
      console.log('   3. 检查服务器控制台是否有错误信息');
    }, 30000);
...
try {
  ...
  console.log('✅ createClient 类型:', typeof graphqlWs.createClient);
} catch (error) {
