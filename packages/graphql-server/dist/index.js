...
// 加载环境变量
dotenv.config();
const { DATABASE_URL = 'postgres://postgres:postgres@127.0.0.1:5432/postgres', PORT = 4000, NODE_ENV = 'development', GRAPHQL_ENDPOINT = '/graphql', PLAYGROUND_ENDPOINT = '/playground', DISABLE_DEFAULT_MUTATIONS = 'false', ENABLE_CORS = 'true', WATCH_PG = 'true', } = process.env;
// 创建数据库连接池
const pgPool = new pg_1.Pool({
    connectionString: DATABASE_URL,
});
// 创建 PostGraphile 中间件
const postgraphileMiddleware = (0, postgraphile_1.postgraphile)(pgPool, process.env.PG_SCHEMA || 'public', {
    ...
    ignoreIndexes: false,
    // 导出模式选项
    exportGqlSchemaPath: NODE_ENV === 'development' ? 'schema.graphql' : undefined,
...
const server = (0, http_1.createServer)(async (req, res) => {
    ...
    try {
        ...
        else if (url.startsWith(PLAYGROUND_ENDPOINT) ||
        ...
        else if (url.startsWith(GRAPHQL_ENDPOINT) ||
...
const startServer = async () => {
    try {
        ...
        server.listen(PORT, () => {
            ...
            console.log(`🎮 GraphQL Playground: http://localhost:${PORT}${PLAYGROUND_ENDPOINT}`);
            console.log(`🔧 PostGraphile GraphiQL: http://localhost:${PORT}/graphiql`);
            console.log('');
