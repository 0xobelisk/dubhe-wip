...
async function init() {
  ...
  const packageId = '0x37fc4f2fb5cc10b04a1848c8a492f897450acde60f654b849856584de9d5a108';
  ...
  const currencyObjectId = '0x47ec13bd4fdcff471bf71554d845146b93b523548a781cbeef1f93809dff806b';

  const callTx = new Transaction();
  const splitAmount = 10000000;
  const selectCoins = await dubhe.selectCoinsWithAmount(splitAmount, '0x2::sui::SUI');
  const bidding_amount = callTx.pure.u64(splitAmount);
  const [coin] = callTx.splitCoins(callTx.object(selectCoins[0]), [bidding_amount]);
