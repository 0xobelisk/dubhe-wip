...
function createEnhancedPlayground(options) {
    return (req, res, _config) => {
        const { url, subscriptionUrl, title = 'Sui Indexer GraphQL Playground', subtitle = '强大的GraphQL API，自动扫描数据库表结构', } = options;
        // 构建GraphiQL配置选项
  ...
  <body>
    ...
    <script>
      try {
        ...
        // 创建 GraphQL 数据获取器
        const fetcher = GraphiQL.createFetcher(${JSON.stringify(graphiqlOptions)});
            ...
            toolbar: {
              additionalContent: [
                ...
                React.createElement('button', {
                  ...
                  className: 'graphiql-toolbar-button',
                  onClick: () => {
                    const schemaUrl = '${url.replace('/graphql', '/graphql/schema')}';
