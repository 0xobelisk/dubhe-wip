...
function generateSimpleComponentCode(projectName: string, componentName: string, valueType: string, type: ComponentType = 'Onchain'): string {
  return `module ${projectName}::${projectName}_${componentName} { 
    ...
    use ${projectName}::${projectName}_dapp_key;
    use ${projectName}::${projectName}_dapp_key::DappKey;
    ...
    public fun register_table(dapp_hub: &mut DappHub, ctx: &mut TxContext) {
        let dapp_key = ${projectName}_dapp_key::new();
    ...
    public fun delete(dapp_hub: &mut DappHub, id: address) {
        let mut key_tuple = vector::empty();
        key_tuple.push_back(to_bytes(&id));
        dapp_service::delete_record<DappKey>(dapp_hub, ${projectName}_dapp_key::new(), get_table_id(), key_tuple);
    }
    ...
    public fun set(dapp_hub: &mut DappHub, id: address, value: ${valueType}) {
        ...
        dapp_service::set_record(dapp_hub, ${projectName}_dapp_key::new(), get_table_id(), key_tuple, value_tuple);
    }
...
${!isSingleValue ? valueNames.map((name, index) => `    public fun has_${name}(dapp_hub: &DappHub${keyParams ? ', ' + keyParams : ''}): bool {
  ...
  const getterSetters = !isSingleValue ? valueNames.map(name => {
    ...
    public fun set_${name}(dapp_hub: &mut DappHub${keyParams ? ', ' + keyParams : ''}, ${name}: ${fieldType}) {
        ...
        dapp_service::set_field(dapp_hub, ${projectName}_dapp_key::new(), get_table_id(), key_tuple, ${index}, value);
    }`;
  }).join('\n\n') : '';
