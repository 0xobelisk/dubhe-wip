  ...
  module test_project::component17 {
  ...
  const TABLE_NAME: vector<u8> = b"component17";
  ...
  public fun get(dapp_hub: &DappHub, entity_id: address): (vector<String>) {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&entity_id));
  ...
  public(package) fun set(dapp_hub: &mut DappHub, entity_id: address, value: vector<String>) {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&entity_id));
  ...
  public fun encode(value: vector<String>): vector<vector<u8>> {
