...
impl DubheGrpcService {
    ...
    async fn build_sql_query(&self, req: &QueryRequest) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        ...
        // FROM clause
        sql.push_str(&format!(" FROM {}", req.table_name));
    ...
    fn build_filter_condition(&self, filter: &FilterCondition) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        ...
        let condition = match operator {
            ...
            FilterOperator::Like => {
                format!("{} LIKE {}", field, self.format_filter_value(&filter.value)?)
            },
            FilterOperator::NotLike => {
                format!("{} NOT LIKE {}", field, self.format_filter_value(&filter.value)?)
            },
            FilterOperator::In => {
                if let Some(FilterValue { value: Some(filter_value::Value::StringList(list)) }) = &filter.value {
                    let values: Vec<String> = list.values.iter().map(|v| format!("'{}'", v.replace("'", "''"))).collect();
                    format!("{} IN ({})", field, values.join(", "))
                } else if let Some(FilterValue { value: Some(filter_value::Value::IntList(list)) }) = &filter.value {
            ...
            FilterOperator::Between => {
                if let Some(FilterValue { value: Some(filter_value::Value::Range(range)) }) = &filter.value {
    ...
    fn format_filter_value(&self, value: &Option<FilterValue>) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        if let Some(filter_value) = value {
            if let Some(v) = &filter_value.value {
                match v {
                    filter_value::Value::StringValue(s) => Ok(format!("'{}'", s.replace("'", "''"))),
    ...
    fn format_range_value_start(&self, value: &Option<value_range::Start>) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        if let Some(v) = value {
            match v {
    ...
    fn format_range_value_end(&self, value: &Option<value_range::End>) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
        if let Some(v) = value {
            match v {
    ...
    fn convert_json_to_prost_value(&self, json_value: &sqlx::types::JsonValue) -> prost_types::Value {
        match json_value {
            ...
            sqlx::types::JsonValue::Array(arr) => {
                let list_values: Vec<prost_types::Value> = arr.iter()
            ...
            sqlx::types::JsonValue::Object(obj) => {
                let struct_fields: std::collections::BTreeMap<String, prost_types::Value> = 
                    obj.iter().map(|(k, v)| {
                        (k.clone(), self.convert_json_to_prost_value(v))
                    }).collect();
                prost_types::Value {
                    kind: Some(prost_types::value::Kind::StructValue(
                        prost_types::Struct { fields: struct_fields }
                    )),
                }
            },
        }
    ...
    async fn get_total_count(&self, database: &Database, req: &QueryRequest) -> Result<i64, Box<dyn std::error::Error + Send + Sync>> {
        let mut count_sql = format!("SELECT COUNT(*) as count FROM {}", req.table_name);
...
impl DubheGrpc for DubheGrpcService {
                ...
                match database.query(&sql).await {
                    Ok(results) => {
                        ...
                        let pagination_info = if let Some(pagination) = &req.pagination {
                            let total_items = if req.include_total_count {
                                self.get_total_count(database, &req).await.unwrap_or(results.len() as i64)
                            } else {
                                results.len() as i64
                            };
                        ...
                        log::info!("✅ gRPC query_table successful: returned {} rows", results.len());
    ...
    type SubscribeTableStream = tokio_stream::wrappers::UnboundedReceiverStream<Result<TableChange, Status>>;
