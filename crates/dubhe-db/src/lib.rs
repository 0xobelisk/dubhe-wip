...
impl DatabaseAsyncRef for DubheDB {
    ...
    async fn object_async_ref(
        ...
        let sui_object_response = self
            ...
            .await?;
        let sui_object_data = sui_object_response
            .into_object()
            .map_err(|e| DBTransportError(SuiSdkError::DataError(e.to_string())))?;
        let object: sui_types::object::Object = sui_object_data.try_into().map_err(|e| {
            DBTransportError(SuiSdkError::DataError(format!(
                "Failed to convert SuiObjectData to Object: {:?}",
                e
            )))
        })?;
        Ok(Some(object))
...
pub fn get_dapp_key_str(package_id: &str) -> String {
    ...
    format!(
        "{}::dapp_key::DappKey",
        package_id.strip_prefix("0x").unwrap()
    )
}
...
pub async fn initialize_cache<ExtDB: DatabaseRef>(
    ...
    let hub_response = client
        .read_api()
        .get_object_with_options(hub_id, options.clone())
        .await
        .unwrap();
    ...
    println!(
        "✅ Dubhe Hub cached, dapp_stores_field_id: {}",
        dapp_stores_field_id
    );

    println!("\n🚀 Step 2: Fetching dapp stores list...");
    // Step 2: 获取 dapp_stores 的动态字段列表（只调用一次）
    let dapp_stores_page = client
        .read_api()
    ...
    // 找到两个需要的 dapp_store
    let origin_dapp_store_info = dapp_stores_page
        .data
        .iter()
        .find(|info| {
            info.name
                .value
                .to_string()
                .contains(&get_dapp_key_str(origin_package_id))
        })
        .unwrap();
    let dubhe_dapp_store_info = dapp_stores_page
        .data
        .iter()
        .find(|info| {
            info.name
                .value
                .to_string()
                .contains(&get_dapp_key_str(orogin_dubhe_package_id))
        })
        .unwrap();
    ...
    println!(
        "  - Origin dapp store: {}",
        origin_dapp_store_info.object_id
    );
    ...
    let dapp_store_ids = vec![
        origin_dapp_store_info.object_id,
        dubhe_dapp_store_info.object_id,
    ];
    let dapp_stores = client
        .read_api()
    ...
    let origin_tables_page = client
        .read_api()
        .get_dynamic_fields(tables_field_ids[0], None, Some(50))
        .await
        .unwrap();
    let dubhe_tables_page = client
        .read_api()
    ...
    println!(
        "✅ Origin package has {} tables",
        origin_tables_page.data.len()
    );
    println!(
        "✅ Dubhe package has {} tables",
        dubhe_tables_page.data.len()
    );
    ...
    if let Some(fee_state_info) = dubhe_tables_page
        .data
        .iter()
    ...
    println!(
        "✅ Found {} total records across all tables",
        all_record_ids.len()
    );
    ...
    println!(
        ...
        cache_db.cache.read().unwrap().objects.len()
    );
}
...
mod tests {
    ...
    async fn can_get_object() {
        ...
        println!(
            "cache hash map length: {:?}",
            cache_db.cache.read().unwrap().sui_object_data.len()
        );
