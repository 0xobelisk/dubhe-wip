...
__export(src_exports, {
  AptosAccountManager: () => AptosAccountManager,
  AptosContractFactory: () => AptosContractFactory,
...
__reExport(src_exports, require("aptos"), module.exports);

// src/dubhe.ts
var import_aptos5 = require("aptos");
...
var getKeyPair = (mnemonics, derivePathParams = {}) => {
  ...
  return import_aptos.AptosAccount.fromDerivePath(derivePath, mnemonics);
};
...
var AptosAccountManager = class {
  ...
  constructor({ mnemonics, secretKey } = {}) {
    ...
    this.currentKeyPair = this.secretKey ? import_aptos2.AptosAccount.fromAptosAccountObject({
...
var getDefaultURL = (networkType = import_aptos3.Network.DEVNET) => {
  switch (networkType) {
    case import_aptos3.Network.LOCAL:
      return {
        fullNode: "http://127.0.0.1:8080",
        faucet: "http://127.0.0.1:8081"
      };
    case import_aptos3.Network.DEVNET:
      return {
        fullNode: "https://fullnode.devnet.aptoslabs.com",
        faucet: "https://faucet.devnet.aptoslabs.com"
      };
    case import_aptos3.Network.TESTNET:
      return {
        fullNode: "https://fullnode.testnet.aptoslabs.com",
        faucet: "https://faucet.testnet.aptoslabs.com"
      };
    case import_aptos3.Network.MAINNET:
      return {
...
} = import_aptos4.TxnBuilderTypes;
var AptosInteractor = class {
  constructor(fullNodeUrls, network) {
    if (fullNodeUrls.length === 0)
      throw new Error("fullNodeUrls must not be empty");
    this.providers = fullNodeUrls.map(
      (url) => new import_aptos4.Provider({ fullnodeUrl: url, indexerUrl: url })
    );
    this.currentProvider = this.providers[0];
    this.currentClient = new import_aptos4.AptosClient(fullNodeUrls[0]);
    this.network = network;
    if (network !== void 0 && network !== import_aptos4.Network.LOCAL && network !== "movementlocal" /* LOCAL */) {
      this.indexerClient = new import_aptos4.IndexerClient(NetworkNameToIndexerAPI[network]);
    }
  }
  ...
  async sendTxWithPayload(signer, sender, payload) {
    for (const provider of this.providers) {
      try {
        ...
        const bcsTxn = import_aptos4.AptosClient.generateBCSTransaction(signer, rawTxn);
  ...
  async requestFaucet(network, accountAddress, amount) {
    ...
    try {
      const faucetClient = new import_aptos4.FaucetClient(
        defaultUrl.fullNode,
        defaultUrl.faucet
      );
...
var Dubhe = class {
   ...
   */
  async requestFaucet(network, accountAddress, amount) {
    if (network === import_aptos5.Network.MAINNET) {
      return false;
    }
  ...
  async signAndSendTxnWithPayload(payload, sender, derivePathParams) {
    ...
    if (typeof sender === "string") {
      sender = new import_aptos5.HexString(sender);
    }
