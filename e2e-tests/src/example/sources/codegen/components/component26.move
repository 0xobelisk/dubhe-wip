  ...
  const TABLE_NAME: vector<u8> = b"component26";
  ...
  public fun get(dapp_hub: &DappHub, entity_id: address): (vector<u64>) {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&entity_id));
    let value_tuple = dapp_service::get_record<DappKey>(dapp_hub, get_table_id(), key_tuple);
    let mut bsc_type = sui::bcs::new(value_tuple);
    let value = sui::bcs::peel_vec_u64(&mut bsc_type);
    (value)
  }

  public(package) fun set(dapp_hub: &mut DappHub, entity_id: address, value: vector<u64>) {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&entity_id));
  ...
  public fun encode(value: vector<u64>): vector<vector<u8>> {
