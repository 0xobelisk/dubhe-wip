  ...
  const TABLE_NAME: vector<u8> = b"component18";
  ...
  public fun get(dapp_hub: &DappHub, entity_id: address): (u64) {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&entity_id));
  ...
  public(package) fun set(dapp_hub: &mut DappHub, entity_id: address, value: u64) {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&entity_id));
