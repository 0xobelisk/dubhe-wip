  // Copyright (c) Obelisk Labs, Inc.
  // SPDX-License-Identifier: Apache-2.0
  #[allow(unused_use)]
  
  /* Autogenerated file. Do not edit manually. */
  
  module example::component12 {

  use sui::bcs::{to_bytes};

  use dubhe::table_id;

  use dubhe::dapp_service::{Self, DappHub};

  use example::dapp_key;

  use example::dapp_key::DappKey;

  use example::direction;

  use example::direction::{Direction};

  const TABLE_NAME: vector<u8> = b"component12";

  public struct Component12 has copy, drop, store {
    player: address,
    value: u32,
  }

  public fun new(player: address, value: u32): Component12 {
    Component12 {
            player,
            value,
        }
  }

  public fun player(self: &Component12): address {
    self.player
  }

  public fun value(self: &Component12): u32 {
    self.value
  }

  public fun update_player(self: &mut Component12, player: address) {
    self.player = player
  }

  public fun update_value(self: &mut Component12, value: u32) {
    self.value = value
  }

  public fun get_table_id(): vector<u8> {
    table_id::encode(table_id::onchain_table_type(), TABLE_NAME)
  }

  public fun get_key_schemas(): vector<vector<u8>> {
    vector[b"Direction"]
  }

  public fun get_value_schemas(): vector<vector<u8>> {
    vector[b"address", b"u32"]
  }

  public fun get_key_names(): vector<vector<u8>> {
    vector[b"direction"]
  }

  public fun get_value_names(): vector<vector<u8>> {
    vector[b"player", b"value"]
  }

  public(package) fun register_table(dapp_hub: &mut DappHub, ctx: &mut TxContext) {
    let dapp_key = dapp_key::new();
    dapp_service::register_table(
            dapp_hub, 
            dapp_key,
            get_table_id(), 
            TABLE_NAME, 
            get_key_schemas(), 
            get_key_names(), 
            get_value_schemas(), 
            get_value_names(), 
            ctx
        );
  }

  public fun has(dapp_hub: &DappHub, direction: Direction): bool {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&direction));
    dapp_service::has_record<DappKey>(dapp_hub, get_table_id(), key_tuple)
  }

  public fun ensure_has(dapp_hub: &DappHub, direction: Direction) {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&direction));
    dapp_service::ensure_has_record<DappKey>(dapp_hub, get_table_id(), key_tuple)
  }

  public fun ensure_not_has(dapp_hub: &DappHub, direction: Direction) {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&direction));
    dapp_service::ensure_not_has_record<DappKey>(dapp_hub, get_table_id(), key_tuple)
  }

  public fun has_player(dapp_hub: &DappHub, direction: Direction): bool {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&direction));
    dapp_service::has_field<DappKey>(dapp_hub, get_table_id(), key_tuple, 0)
  }

  public fun ensure_has_player(dapp_hub: &DappHub, direction: Direction) {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&direction));
    dapp_service::ensure_has_field<DappKey>(dapp_hub, get_table_id(), key_tuple, 0)
  }

  public fun ensure_not_has_player(dapp_hub: &DappHub, direction: Direction) {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&direction));
    dapp_service::ensure_not_has_field<DappKey>(dapp_hub, get_table_id(), key_tuple, 0)
  }

  public fun has_value(dapp_hub: &DappHub, direction: Direction): bool {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&direction));
    dapp_service::has_field<DappKey>(dapp_hub, get_table_id(), key_tuple, 1)
  }

  public fun ensure_has_value(dapp_hub: &DappHub, direction: Direction) {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&direction));
    dapp_service::ensure_has_field<DappKey>(dapp_hub, get_table_id(), key_tuple, 1)
  }

  public fun ensure_not_has_value(dapp_hub: &DappHub, direction: Direction) {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&direction));
    dapp_service::ensure_not_has_field<DappKey>(dapp_hub, get_table_id(), key_tuple, 1)
  }

  public(package) fun delete(dapp_hub: &mut DappHub, direction: Direction) {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&direction));
    dapp_service::delete_record<DappKey>(dapp_hub, dapp_key::new(), get_table_id(), key_tuple);
  }

  public fun get_player(dapp_hub: &DappHub, direction: Direction): address {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&direction));
    let value = dapp_service::get_field<DappKey>(dapp_hub, get_table_id(), key_tuple, 0);
    let mut bsc_type = sui::bcs::new(value);
    let player = sui::bcs::peel_address(&mut bsc_type);
    player
  }

  public(package) fun set_player(dapp_hub: &mut DappHub, direction: Direction, player: address) {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&direction));
    let value = to_bytes(&player);
    dapp_service::set_field(dapp_hub, dapp_key::new(), get_table_id(), key_tuple, 0, value);
  }

  public fun get_value(dapp_hub: &DappHub, direction: Direction): u32 {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&direction));
    let value = dapp_service::get_field<DappKey>(dapp_hub, get_table_id(), key_tuple, 1);
    let mut bsc_type = sui::bcs::new(value);
    let value = sui::bcs::peel_u32(&mut bsc_type);
    value
  }

  public(package) fun set_value(dapp_hub: &mut DappHub, direction: Direction, value: u32) {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&direction));
    let value = to_bytes(&value);
    dapp_service::set_field(dapp_hub, dapp_key::new(), get_table_id(), key_tuple, 1, value);
  }

  public fun get(dapp_hub: &DappHub, direction: Direction): (address, u32) {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&direction));
    let value_tuple = dapp_service::get_record<DappKey>(dapp_hub, get_table_id(), key_tuple);
    let mut bsc_type = sui::bcs::new(value_tuple);
    let player = sui::bcs::peel_address(&mut bsc_type);
    let value = sui::bcs::peel_u32(&mut bsc_type);
    (player, value)
  }

  public(package) fun set(dapp_hub: &mut DappHub, direction: Direction, player: address, value: u32) {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&direction));
    let value_tuple = encode(player, value);
    dapp_service::set_record(dapp_hub, dapp_key::new(), get_table_id(), key_tuple, value_tuple);
  }

  public fun get_struct(dapp_hub: &DappHub, direction: Direction): Component12 {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&direction));
    let value_tuple = dapp_service::get_record<DappKey>(dapp_hub, get_table_id(), key_tuple);
    decode(value_tuple)
  }

  public(package) fun set_struct(dapp_hub: &mut DappHub, direction: Direction, component12: Component12) {
    let mut key_tuple = vector::empty();
    key_tuple.push_back(to_bytes(&direction));
    let value_tuple = encode_struct(component12);
    dapp_service::set_record(dapp_hub, dapp_key::new(), get_table_id(), key_tuple, value_tuple);
  }

  public fun encode(player: address, value: u32): vector<vector<u8>> {
    let mut value_tuple = vector::empty();
    value_tuple.push_back(to_bytes(&player));
    value_tuple.push_back(to_bytes(&value));
    value_tuple
  }

  public fun encode_struct(component12: Component12): vector<vector<u8>> {
    encode(component12.player, component12.value)
  }

  public fun decode(data: vector<u8>): Component12 {
    let mut bsc_type = sui::bcs::new(data);
    let player = sui::bcs::peel_address(&mut bsc_type);
    let value = sui::bcs::peel_u32(&mut bsc_type);
    Component12 {
            player,
            value,
        }
  }
}
