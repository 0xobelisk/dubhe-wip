...
services:
  graphql-server:
    ...
    restart: unless-stopped
    command:
      ['npx', 'dubhe-graphql-server', 'start', '--env', 'production', '--cors', '--subscriptions']
```
...
export const dbConfig = {
  ...
  replicas: [process.env.DATABASE_REPLICA_1_URL, process.env.DATABASE_REPLICA_2_URL].filter(Boolean)
};
```
