...
const transactions = await dubhe.getTransactions({
  ...
  sender: "0x123...", // Filter by sender address
  digest: "tx_hash", // Filter by transaction hash
  checkpoint: 12345, // Filter by checkpoint number
  orderBy: ["ID_ASC"], // Sorting options
});

// Get a single transaction by digest
const transaction = await dubhe.getTransaction("tx_hash");

// Wait for transaction to be indexed
const indexedTx = await dubhe.waitForIndexerTransaction("tx_hash", {
  checkInterval: 100, // Optional: Polling interval in ms (default: 100)
  timeout: 30000, // Optional: Maximum wait time in ms (default: 30000)
  maxRetries: 300, // Optional: Maximum number of retries (default: 300)
});
```
...
const events = await dubhe.getEvents({
  ...
  sender: "0x123...", // Filter by sender address
  digest: "tx_hash", // Filter by transaction hash
  checkpoint: "12345", // Filter by checkpoint
  orderBy: ["ID_ASC"], // Sorting options
});
```
...
const schemas = await dubhe.getSchemas({
  name: "schema_name", // Filter by schema name
  key1: "key1_value", // Filter by key1
  key2: "key2_value", // Filter by key2
  is_removed: false, // Filter by removal status
  value: "value", // Filter by value
  last_update_checkpoint: "12345", // Filter by last update checkpoint
  last_update_digest: "tx_hash", // Filter by last update transaction
  first: 10, // Number of items per page
  after: "cursor", // Cursor for pagination
  orderBy: ["ID_ASC"], // Sorting options
  jsonOrderBy: [{ path: "field", direction: "ASC" }], // JSON path sorting options
});

// Access the results
console.log(schemas.edges); // Array of schema data
console.log(schemas.pageInfo); // Pagination information
console.log(schemas.totalCount); // Total number of items
```
...
const item = await dubhe.getStorageItem({
  ...
  key2: "key2_value",
  is_removed: false,
  last_update_checkpoint: "12345",
  last_update_digest: "tx_hash",
  value: "value",
});
...
const websocket = await dubhe.subscribe(
  [
    ...
    },
    {
      ...
      name: "event_name",
      // Optional: Filter events by sender address
      // If provided, only events from this address will be returned
      // If omitted, events from all addresses will be included
...
// The websocket connection can be closed when no longer needed
websocket.close();
```
...
- For Transactions: `ID_ASC`, `ID_DESC`, `CREATED_AT_ASC`, `CREATED_AT_DESC`, `CHECKPOINT_ASC`, `CHECKPOINT_DESC`
- For Events: `ID_ASC`, `ID_DESC`, `CREATED_AT_ASC`, `CREATED_AT_DESC`, `CHECKPOINT_ASC`, `CHECKPOINT_DESC`
- For Schemas:
  - `ID_ASC`, `ID_DESC`
...
You can also use JSON path ordering for more complex sorting:

```typescript
// Example of JSON path ordering
const schemas = await dubhe.getSchemas({
  first: 10,
  jsonOrderBy: [
...
```typescript
// Wait for transaction to be indexed before proceeding
try {
  const tx = await dubhe.signAndSendTxn({
    /* ... */
  });
  const indexedTx = await dubhe.waitForIndexerTransaction(tx.digest, {
    timeout: 60000, // Increase timeout for slower networks
    checkInterval: 200, // Adjust polling interval as needed
  });
...
let cursor = null;
while (hasMore) {
  const page = await dubhe.getSchemas({
    first: 50,
    after: cursor,
  });
